...
export default function GroupPage({ params }: { params: Promise<{ groupId: string }> }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    await fetch("/api/expenses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, group_id: groupId }) });
    setSaving(false); setShowAdd(false);
    setForm(f => ({ ...f, vendor: "", amount: "", sub_category: "", note: "", date: new Date().toISOString().split("T")[0] }));
    load();
  };

  const startEdit = (item: any) => { setEditingId(item.id); setEditForm({ vendor: item.vendor, amount: item.amount, category: item.category, sub_category: item.sub_category || "", note: item.note || "", date: item.date }); };
  ...
  // Bar chart max
  const barMax = Math.max(...trend.map(t => t.total || 0), 1);

  // Trends: top 3 cats by total across all months
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e8f0", fontFamily: "'IBM Plex Sans Thai', Sarabun, sans-serif" }}>
      ...
      <div style={{ padding: "16px 16px 0", position: "sticky", top: 0, zIndex: 20, background: "#0a0a0f" }}>
        ...
        {/* Month selector - only show on tabs that need it */}
        {[0,1,3].includes(tab) && (
        ...
        {/* Tabs */}
        <div style={{ display: "flex", gap: 4, marginBottom: 0, overflowX: "auto", scrollbarWidth: "none" }}>
          {TABS.map((t, i) => (
      ...
      <div style={{ padding: "16px 16px 100px" }}>
        {loading ? <div style={{ textAlign: "center", padding: 60, color: "#4b5563" }}>กำลังโหลด...</div> : <>
          ...
          {/* ── Tab 0: รายการ ── */}
          {tab === 0 && <>
            <div style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.25),rgba(139,92,246,0.15))", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 16, padding: "16px 20px", marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 3 }}>รวมเดือนนี้</div>
              <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-1px" }}>{thb(summary?.total || 0)}</div>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 6, marginBottom: 12, scrollbarWidth: "none" }}>
              {[{ id: "all", label: "ทั้งหมด", color: "#6366f1" }, ...cats].map(c => (
                <button key={c.id} onClick={() => setFilter(c.id)}
                  style={{ whiteSpace: "nowrap", padding: "5px 12px", borderRadius: 20, border: "1px solid", fontSize: 12, background: filter === c.id ? `${c.color}25` : "transparent", borderColor: filter === c.id ? c.color : "rgba(255,255,255,0.1)", color: filter === c.id ? c.color : "#9ca3af", flexShrink: 0 }}>
                  {c.label}
                </button>
              ))}
            </div>
                      ...
                      {!isEditing ? (
                        <div style={{ padding: "13px 15px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                          ...
                          <div style={{ textAlign: "right", flexShrink: 0 }}>
                            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 5 }}>{thb(item.amount)}</div>
                        ...
                        <div style={{ padding: "14px", background: "rgba(99,102,241,0.08)", borderLeft: "3px solid #6366f1" }}>
                          {[{label:"ร้านค้า",key:"vendor",type:"text"},{label:"จำนวน",key:"amount",type:"number"},{label:"วันที่",key:"date",type:"date"},{label:"หมายเหตุ",key:"note",type:"text"}].map(f => (
          ...
          {tab === 1 && <>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 4 }}>รวมเดือนนี้</div>
              <div style={{ fontSize: 28, fontWeight: 800 }}>{thb(pieTotal)}</div>
            </div>

            {pieData.length === 0 ? (
              <div style={{ textAlign: "center", padding: 40, color: "#4b5563" }}>ไม่มีข้อมูล</div>
            ) : <>
              {/* SVG Pie */}
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
                <svg width="220" height="220" viewBox="0 0 220 220">
                  ...
                  <circle cx="110" cy="110" r="50" fill="#0a0a0f" />
                </svg>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {pieData.sort((a,b) => b.total - a.total).map(cat => (
                  <div key={cat.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: `${cat.color}12`, border: `1px solid ${cat.color}30`, borderRadius: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: "#6b7280" }}>{(cat.total / pieTotal * 100).toFixed(1)}%</div>
                      </div>
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: cat.color }}>{thb(cat.total)}</div>
                  </div>
                ))}
              </div>
              ...
              {summary?.byCategory?.personal?.bySubCategory && Object.keys(summary.byCategory.personal.bySubCategory).length > 0 && (
                <div style={{ marginTop: 16, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 14, padding: "14px" }}>
                    ...
                    return (
                      <div key={sub} style={{ marginBottom: 8 }}>
                        ...
                        <div style={{ height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
                          <div style={{ height: 4, background: "#6366f1", borderRadius: 2, width: `${pct}%` }} />
                        </div>
                      </div>
                    );
                  })}
          ...
          {/* ── Tab 2: Bar ── */}
          {tab === 2 && <>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#9ca3af", marginBottom: 16 }}>ค่าใช้จ่ายรายเดือน (6 เดือนล่าสุด)</div>
            {trend.length === 0 ? <div style={{ textAlign: "center", padding: 40, color: "#4b5563" }}>ไม่มีข้อมูล</div> : <>
              <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 6, height: 180, marginBottom: 8 }}>
                {trend.map((t, i) => (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%" }}>
                    <div style={{ flex: 1, display: "flex", alignItems: "flex-end", width: "100%" }}>
                      <div style={{ width: "100%", background: "linear-gradient(to top,#6366f1,#8b5cf6)", borderRadius: "4px 4px 0 0", height: `${(t.total / barMax) * 100}%`, minHeight: t.total > 0 ? 4 : 0, transition: "height 0.4s" }} />
                    </div>
                    <div style={{ fontSize: 10, color: "#6b7280", marginTop: 4, textAlign: "center" }}>{shortMonth(t.month)}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 16 }}>
                {trend.map((t, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{new Date(t.month + "-01").toLocaleDateString("th-TH", { month: "long", year: "numeric" })}</div>
                      <div style={{ fontSize: 11, color: "#6b7280" }}>{t.count} รายการ</div>
                    </div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: "#a5b4fc" }}>{thb(t.total)}</div>
                  </div>
                ))}
              </div>

              {/* Stacked breakdown */}
              <div style={{ marginTop: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "14px" }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#9ca3af", marginBottom: 12 }}>แยกตามหมวด</div>
                {cats.map(cat => {
                  ...
                  return (
                    <div key={cat.id} style={{ marginBottom: 14 }}>
                      <div style={{ fontSize: 12, color: cat.color, fontWeight: 600, marginBottom: 6 }}>{cat.label}</div>
                      <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 40 }}>
                        {trend.map((t, i) => {
                          const val = t.byCategory?.[cat.id]?.total || 0;
                          return (
                            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%" }}>
                              <div style={{ flex: 1, display: "flex", alignItems: "flex-end", width: "100%" }}>
                                <div style={{ width: "100%", background: cat.color, borderRadius: "3px 3px 0 0", height: `${(val / max) * 100}%`, minHeight: val > 0 ? 2 : 0, opacity: 0.7 }} />
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div style={{ display: "flex", gap: 4 }}>
                        {trend.map((t, i) => (
                          <div key={i} style={{ flex: 1, fontSize: 9, color: "#4b5563", textAlign: "center" }}>{shortMonth(t.month)}</div>
                        ))}
                      </div>
                    </div>
                  );
          ...
          {tab === 3 && <>
            ...
            {editBudget && (
              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "14px", marginBottom: 16 }}>
                <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 12 }}>ตั้งงบแต่ละหมวด (บาท/เดือน)</div>
                {cats.map(cat => (
                ...
                <button onClick={saveBudgets} style={{ width: "100%", padding: "10px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#6366f1,#8b5cf6)", color: "white", fontSize: 14, fontWeight: 600, marginTop: 4 }}>บันทึกงบ</button>
              </div>
            )}
                ...
                return (
                  <div key={cat.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "14px 16px" }}>
                    ...
                    {budget > 0 ? <>
                      ...
                      <div style={{ fontSize: 11, color: "#4b5563" }}>ยังไม่ได้ตั้งงบ — กด "ตั้งงบ" เพื่อเพิ่ม</div>
                    )}
                  </div>
                );
              })}
            ...
            {Object.values(budgets).some(v => v > 0) && (
              <div style={{ marginTop: 16, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 14, padding: "14px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: "#a5b4fc" }}>งบรวม</span>
                  <span style={{ fontSize: 14, fontWeight: 700 }}>{thb(Object.values(budgets).reduce((s, v) => s + v, 0))}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 13, color: "#9ca3af" }}>ใช้ไปแล้ว</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#a5b4fc" }}>{thb(summary?.total || 0)}</span>
                </div>
              </div>
            )}
          </>}
          ...
          {/* ── Tab 4: Trends ── */}
          {tab === 4 && <>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#9ca3af", marginBottom: 16 }}>แนวโน้ม 6 เดือนล่าสุด</div>

            {trend.length === 0 ? <div style={{ textAlign: "center", padding: 40, color: "#4b5563" }}>ไม่มีข้อมูล</div> : <>
              {/* Line chart per top cat */}
              {catTotals.map(cat => {
                const vals = trend.map(t => t.byCategory?.[cat.id]?.total || 0);
                const max = Math.max(...vals, 1);
                ...
                return (
                  <div key={cat.id} style={{ background: `${cat.color}10`, border: `1px solid ${cat.color}25`, borderRadius: 14, padding: "14px 16px", marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: cat.color }}>{cat.label}</div>
                      <div style={{ fontSize: 12, color: "#6b7280" }}>รวม 6 เดือน: {thb(cat.total)}</div>
                    </div>
                    <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
                      <polyline points={pts} fill="none" stroke={cat.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      {vals.map((v, i) => (
                        <circle key={i} cx={pad + i * ((W - pad*2) / (vals.length - 1))} cy={H - pad - (v / max) * (H - pad*2)} r="3" fill={cat.color} />
                      ))}
                    </svg>
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                      {trend.map((t, i) => <div key={i} style={{ fontSize: 9, color: "#4b5563", flex: 1, textAlign: "center" }}>{shortMonth(t.month)}</div>)}
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
                      {vals.map((v, i) => <div key={i} style={{ fontSize: 10, color: v > 0 ? cat.color : "#374151", flex: 1, textAlign: "center", fontWeight: v > 0 ? 600 : 400 }}>{v > 0 ? thb(v).replace("฿","") : "-"}</div>)}
                    </div>
                  </div>
                );
              })}
              ...
              {trend.length >= 2 && (
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: "14px 16px" }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#9ca3af", marginBottom: 10 }}>เดือนล่าสุด vs เดือนก่อน</div>
                  {(() => {
                    const last = trend[trend.length - 1];
                    const prev = trend[trend.length - 2];
                    const diff = last.total - prev.total;
                    const pct = prev.total > 0 ? (diff / prev.total * 100).toFixed(1) : "0";
                    const up = diff > 0;
                    return (
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div style={{ fontSize: 22, fontWeight: 800 }}>{thb(last.total)}</div>
                          <div style={{ fontSize: 11, color: "#6b7280" }}>{shortMonth(last.month)}</div>
                        </div>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ fontSize: 20, color: up ? "#ef4444" : "#10b981" }}>{up ? "↑" : "↓"}</div>
                          <div style={{ fontSize: 12, fontWeight: 600, color: up ? "#ef4444" : "#10b981" }}>{up ? "+" : ""}{pct}%</div>
                          <div style={{ fontSize: 10, color: "#6b7280" }}>{thb(Math.abs(diff))}</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 22, fontWeight: 800, color: "#6b7280" }}>{thb(prev.total)}</div>
                          <div style={{ fontSize: 11, color: "#6b7280" }}>{shortMonth(prev.month)}</div>
                        </div>
                      </div>
                    );
                  })()}
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }}
          ...
          <div style={{ background: "#13131f", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "20px 20px 0 0", padding: "22px 18px 36px", width: "100%", maxWidth: 480 }}
            onClick={e => e.stopPropagation()}>
            <div style={{ width: 36, height: 4, background: "rgba(255,255,255,0.2)", borderRadius: 2, margin: "0 auto 18px" }} />
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 16 }}>เพิ่มรายการใหม่</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={save} disabled={saving} style={{ flex: 2, padding: 11, borderRadius: 11, border: "none", background: saving?"#4b5563":"linear-gradient(135deg,#6366f1,#8b5cf6)", color: "white", fontSize: 14, fontWeight: 600 }}>
